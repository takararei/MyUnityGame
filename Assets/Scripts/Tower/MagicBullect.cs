...
public class MagicBullect : Bullect
{
    ...
    protected override void BullectMove()//这个需要持续一段时间
    {
        ...
        if (attackTimer < attackTime)
        {
            ...
            distance = Vector3.Distance(bsTower.transform.position - new Vector3(0, 0, 2), targetTrans.position);
            bullectWidth = distance / 2;
            transform.position = new Vector3((targetTrans.position.x + bsTower.transform.position.x) / 2,
                (targetTrans.position.y + bsTower.transform.position.y) / 2, 0);
